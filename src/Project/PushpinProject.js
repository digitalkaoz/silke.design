...
export default Project => {
  return class PushpinProject extends Component {
    addPushpin() {
      let rect = this.container.parentNode.getBoundingClientRect();
      let topOffset = rect.top + scrollTop();
      let bottomOffset = topOffset + this.container.parentNode.offsetHeight;

      if (this.pushpin) {
        this.pushpin.updateOptions({
          top: topOffset,
          bottom: bottomOffset
        });
      } else {
        this.pushpin = new Pushpin(this.container, {
          top: topOffset,
          bottom: bottomOffset
        });
      }
